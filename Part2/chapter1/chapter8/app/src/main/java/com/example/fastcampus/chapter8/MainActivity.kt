...
class MainActivity : AppCompatActivity(),OnMapReadyCallback {
    ...
    private val restaurantsListAdapter = RestaurantsListAdapter{
        collapseBottomSheet()
        //카메라 움직임 onclick 변수를 LatLng로 바꿔줘야함
        moveCamera(it,17.0)
    }
                    ...
                    SearchRepository.getGoodRestaurants(query).enqueue(object : Callback<SearchResult>{
                        override fun onResponse(call: Call<SearchResult>, response: Response<SearchResult>) {
                            ...
                            if(searchItemList.isEmpty()){
                                ...
                                Toast.makeText(this@MainActivity,"오류가 발생 했습니다.",Toast.LENGTH_SHORT).show()
                                return
                            }
                            ...
                            markers = searchItemList.map {
                                Marker(Tm128(it.mapx.toDouble(),it.mapy.toDouble()).toLatLng()).apply {
                                    val changeTitle = it.title.replace("<b>","").replace("</b>","").replace("amp;","")
                                    it.title = changeTitle
                                    captionText = changeTitle
                                    map = naverMap
                                }
                            }

                            restaurantsListAdapter.setData(searchItemList)
                            collapseBottomSheet()
