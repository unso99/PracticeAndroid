package com.example.fastcampus.part3.face_recognition.recognition
...
internal class FaceAnalyzer(
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        val face = faces.firstOrNull() //성공을 하면 얼굴 하나만 가져옴
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.UnDetect) {
                ...
                listener?.detectProgress(50F, "오른쪽눈만 깜빡여주세요.")
            ...
            //얼굴인식 상태가 잘못되었을때 즉 처음과 끝이 아닐때
            detectStatus = FaceAnalyzerStatus.UnDetect
            listener?.notDetect()
            listener?.detectProgress(0F, "얼굴인식을 하지 못했습니다.\n처음으로 돌아갑니다.")
        }
